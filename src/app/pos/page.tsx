...
export default function POSPage() {
  ...
  const [viewSaleDetail, setViewSaleDetail] = useState<any>(null)
  const [isViewSaleDetailOpen, setIsViewSaleDetailOpen] = useState(false)
    ...
    <div className="flex h-screen flex-col lg:flex-row bg-gray-50 overflow-hidden font-sans selection:bg-orange-200 relative">
      ...
      <aside className="hidden lg:flex w-[100px] sm:w-[120px] flex-col items-center bg-gray-900 shadow-2xl py-6 z-30 shrink-0 overflow-y-auto custom-scrollbar">
        ...
        <div className="flex flex-col gap-6 flex-1 w-full px-2">
          <button onClick={() => document.getElementById('pos-search-input')?.focus()} className="flex flex-col items-center gap-2 text-gray-400 hover:text-white transition-colors group">
             <div className="p-4 rounded-xl bg-gray-800 group-hover:bg-gray-700"><Search className="w-6 h-6" /></div>
             <span className="text-xs font-medium">Search</span>
          </button>
          <button onClick={() => setIsExpenseOpen(true)} className="flex flex-col items-center gap-2 text-gray-400 hover:text-green-400 transition-colors group">
             <div className="p-4 rounded-xl bg-gray-800 group-hover:bg-green-400/20"><Wallet className="w-6 h-6" /></div>
             <span className="text-xs font-medium">Expense</span>
          </button>
          <button onClick={() => setIsReturnOpen(true)} className="flex flex-col items-center gap-2 text-gray-400 hover:text-blue-400 transition-colors group">
             <div className="p-4 rounded-xl bg-gray-800 group-hover:bg-blue-400/20"><RotateCcw className="w-6 h-6" /></div>
             <span className="text-xs font-medium">Return</span>
          </button>
          <button onClick={handleOpenRecentSales} className="flex flex-col items-center gap-2 text-gray-400 hover:text-indigo-400 transition-colors group">
             <div className="p-4 rounded-xl bg-gray-800 group-hover:bg-indigo-400/20"><History className="w-6 h-6" /></div>
             <span className="text-xs font-medium">History</span>
          </button>
          <button onClick={openSlideOutSummary} className="flex flex-col items-center gap-2 text-gray-400 hover:text-purple-400 transition-colors group">
             <div className="p-4 rounded-xl bg-gray-800 group-hover:bg-purple-400/20"><Printer className="w-6 h-6" /></div>
             <span className="text-xs font-medium text-center leading-tight">Z-Report</span>
          </button>
        </div>
        ...
        <div className="mt-auto flex flex-col gap-6 items-center w-full px-2">
           {!isOnline && (
             <div className="text-red-500 animate-pulse flex flex-col items-center" title="Offline Mode">
               ...
               <span className="text-xs mt-1 font-bold">OFFLINE</span>
             </div>
           )}
            ...
            {filteredProducts.map(product => {
              ...
              return (
                <button 
                  ...
                  className={`relative h-full flex flex-col text-center rounded-3xl overflow-hidden transition-all duration-300 bg-white group ${product.isOutOfStock ? 'opacity-70 cursor-not-allowed grayscale-[0.8] border-gray-200' : 'hover:shadow-[0_15px_30px_rgba(0,0,0,0.08)] hover:-translate-y-1 shadow-sm border border-gray-100'}`}
                >
                  ...
                  <div className="p-2 md:p-3 flex-1 flex flex-col items-center justify-center w-full bg-white rounded-b-3xl">
                    ...
                    {product.hasVariants && (
                      <div className="mt-auto w-full flex items-center justify-center gap-1.5 bg-orange-100/80 py-1.5 rounded-lg text-orange-700 shadow-sm transition-all duration-300 group-hover:bg-orange-100 ring-1 ring-orange-200/50">
                        <span className="text-[10px] font-black uppercase tracking-widest">Variants</span>
      ...
      <div className={`${mobileTab === 'cart' ? "fixed inset-0 pt-0 pb-0 z-40 flex w-full" : "hidden lg:flex"} lg:relative lg:w-[420px] lg:pb-0 bg-white flex-col h-full shrink-0 lg:z-20 shadow-2xl border-l border-gray-100`}>
        ...
        <div className="px-3 py-2 flex flex-row items-center gap-2 border-b border-gray-100 bg-gray-50/50">
          <div className="bg-white p-1 flex rounded-lg border border-gray-200 shadow-sm shrink-0">
            {['Takeaway', 'Dine-In'].map(type => (
              <button 
                key={type}
                className={`px-3 py-1.5 text-sm font-bold rounded-md transition-all duration-200 ${orderType === type ? 'bg-gray-900 text-white shadow-sm' : 'text-gray-500 hover:text-gray-900 hover:bg-gray-50'}`}
          ...
          <button onClick={() => setIsCustomerSelectOpen(true)} className="flex items-center justify-between w-full px-3 py-2 bg-white rounded-lg border border-gray-200 hover:border-orange-300 transition-colors group shadow-sm overflow-hidden">
            ...
            cart.map(item => (
              <div key={item.id} className="bg-white p-2 rounded-lg border border-gray-100 shadow-sm flex flex-col gap-1 group relative hover:border-gray-200 transition-colors">
                ...
                <div className="flex items-center justify-between pt-2">
                  ...
                  }} className="text-xs font-bold text-gray-400 hover:text-orange-500 uppercase tracking-wider flex items-center gap-1.5 transition-colors">
                    <Edit className="w-3.5 h-3.5" /> Note
                  </button>
                  
                  <div className="flex items-center gap-3">
                    <div className="flex items-center bg-gray-50 rounded-lg p-1 border border-gray-200">
                      <button onClick={() => updateQuantity(item.id, -1)} className="w-8 h-8 flex items-center justify-center rounded-md bg-white text-gray-500 hover:text-orange-600 shadow-sm transition-colors">
                        <Minus className="w-4 h-4" />
                      </button>
                      <button onClick={() => { setNumpadItem(item); setNumpadValue(String(item.quantity)); }} className="w-10 text-center text-base font-black text-gray-900 hover:text-orange-600">{item.quantity}</button>
                      <button onClick={() => updateQuantity(item.id, 1)} className="w-8 h-8 flex items-center justify-center rounded-md bg-white text-gray-500 hover:text-orange-600 shadow-sm transition-colors">
                        <Plus className="w-4 h-4" />
                      </button>
                    </div>
                    <button onClick={() => removeFromCart(item.id)} className="w-10 h-10 flex items-center justify-center rounded-lg bg-red-50 text-red-500 hover:bg-red-500 hover:text-white transition-colors">
      ...
      <Dialog open={isPaymentModalOpen} onOpenChange={setIsPaymentModalOpen}>
        <DialogContent className="sm:max-w-2xl p-0 overflow-hidden rounded-2xl border-0 shadow-2xl bg-gray-50 max-h-[95vh] sm:max-h-[90vh] flex flex-col">
          <form onSubmit={processPayment} className="flex flex-col h-full overflow-hidden">
            <div className="p-3 sm:p-4 pr-12 bg-white border-b flex justify-between items-center shrink-0">
              <DialogTitle className="text-lg sm:text-xl font-black text-gray-900">Complete Payment</DialogTitle>
              <div className="text-xl sm:text-2xl font-black text-orange-600">Rs. {grandTotal.toFixed(2)}</div>
            </div>
            <div className="p-3 sm:p-4 bg-gray-50 flex flex-col sm:flex-row gap-3 sm:gap-4 overflow-y-auto custom-scrollbar flex-1">
              {/* Left Side: Payment Methods and Inputs */}
              <div className="flex-1 bg-white p-3 sm:p-4 rounded-xl border flex flex-col gap-3 sm:space-y-4 shrink-0">
                <div className="grid grid-cols-4 sm:grid-cols-2 gap-1.5 sm:gap-2">
                  {["Cash", "Card", "Bank Transfer", "Split"].map((method) => (
                    <button
                      key={method} type="button"
                      onClick={() => setPaymentMethod(method as any)}
                      className={`h-10 sm:h-10 rounded-lg sm:rounded-xl border-2 font-bold text-xs sm:text-base transition-all flex items-center justify-center text-center leading-tight p-1 ${paymentMethod === method ? 'border-orange-500 bg-orange-100 text-orange-700 shadow-sm ring-1 ring-orange-500' : 'border-gray-200 bg-white text-gray-600 hover:border-orange-300'}`}
                    >
                ...
                {paymentMethod === "Cash" && (
                  <div className="space-y-3 sm:space-y-4">
                    <div className="grid grid-cols-2 gap-1.5 sm:gap-2">
                      <Button type="button" variant="outline" className="col-span-2 h-9 sm:h-10 text-xs sm:text-sm font-bold bg-green-50 text-green-700 hover:bg-green-100 border-green-200" onClick={() => setTenderedCash(grandTotal.toFixed(2))}>Exact: Rs. {grandTotal.toFixed(2)}</Button>
                      <Button type="button" variant="outline" className="h-9 sm:h-10 text-xs sm:text-sm font-bold" onClick={() => setTenderedCash("500")}>Rs. 500</Button>
                      <Button type="button" variant="outline" className="h-9 sm:h-10 text-xs sm:text-sm font-bold" onClick={() => setTenderedCash("1000")}>Rs. 1000</Button>
                      <Button type="button" variant="outline" className="h-9 sm:h-10 text-xs sm:text-sm font-bold" onClick={() => setTenderedCash("2000")}>Rs. 2000</Button>
                      <Button type="button" variant="outline" className="h-9 sm:h-10 text-xs sm:text-sm font-bold" onClick={() => setTenderedCash("5000")}>Rs. 5000</Button>
                    </div>
                    <div className="space-y-1">
                      <Label className="font-bold text-gray-700 text-xs sm:text-sm">Tendered Cash (Rs.)</Label>
                      <Input ref={tenderedCashInputRef} type="number" step="0.01" value={tenderedCash} onChange={e => setTenderedCash(e.target.value)} onFocus={() => setActiveInput("tenderedCash")} placeholder="0.00" className="h-10 sm:h-12 font-black text-lg sm:text-xl" required />
                    </div>
                    {parseFloat(tenderedCash) >= grandTotal && (
                      <div className="text-xs sm:text-sm font-black text-green-600 text-right">
                        Change: <span className="text-base sm:text-lg">Rs. {(parseFloat(tenderedCash) - grandTotal).toFixed(2)}</span>
                      </div>
                    )}
                  </div>
                )}
                ...
                {paymentMethod === "Split" && (
                  <div className="space-y-2 sm:space-y-3">
                    <div className="space-y-1">
                      <Label className="font-bold text-gray-700 text-xs sm:text-sm">Cash Amount (Rs.)</Label>
                      <Input type="number" step="0.01" value={splitCash} onChange={e => setSplitCash(e.target.value)} onFocus={() => setActiveInput("splitCash")} placeholder="0.00" className="h-10 sm:h-12 font-black text-lg sm:text-xl" required />
                    </div>
                    <div className="space-y-1">
                      <Label className="font-bold text-gray-700 text-xs sm:text-sm">Card Amount (Rs.)</Label>
                      <Input type="number" step="0.01" value={splitCard} onChange={e => setSplitCard(e.target.value)} onFocus={() => setActiveInput("splitCard")} placeholder="0.00" className="h-10 sm:h-12 font-black text-lg sm:text-xl" required />
                    </div>
                    <div className="text-xs sm:text-sm font-bold text-gray-500 text-right">
                ...
                {(paymentMethod === "Card" || paymentMethod === "Bank Transfer") && (
                  <div className="py-4 sm:py-6 text-center text-gray-500 text-xs sm:text-sm font-medium">
              ...
              {/* Right Side: Numpad */}
              <div className="flex-1 bg-white p-3 sm:p-4 rounded-xl border flex items-center justify-center shrink-0">
                <div className="grid grid-cols-3 gap-1.5 sm:gap-2 w-full max-w-[280px]">
                  {['1', '2', '3', '4', '5', '6', '7', '8', '9', 'C', '0', 'X'].map(key => (
                    <button 
                      key={key} type="button"
                      onClick={() => handleNumpadClick(key)}
                      className={`h-10 sm:h-12 rounded-lg sm:rounded-xl font-bold text-lg sm:text-xl transition-colors shadow-sm ${
                    ...
                    >
                      {key === 'X' ? <Delete className="w-5 h-5 sm:w-6 sm:h-6 mx-auto" /> : key}
                    </button>
                  ))}
                </div>
              </div>
            </div>
            <div className="p-3 sm:p-4 bg-white border-t flex gap-2 sm:gap-3 shrink-0">
              <Button type="button" variant="outline" className="flex-1 h-10 sm:h-12 text-sm sm:text-base font-bold rounded-lg sm:rounded-xl" onClick={() => setIsPaymentModalOpen(false)}>Cancel</Button>
              <Button type="submit" disabled={isProcessing} className="flex-1 h-10 sm:h-12 text-sm sm:text-base font-bold rounded-lg sm:rounded-xl bg-orange-500 hover:bg-orange-600 text-white shadow-lg disabled:opacity-70">
      ...
      <Dialog open={paymentSuccess} onOpenChange={() => { setPaymentSuccess(false); setCart([]); setDiscountType("NONE"); setDiscountValue(0) }}>
        <DialogContent className="sm:max-w-md p-8 overflow-hidden rounded-2xl border-0 shadow-2xl bg-white text-center">
          <div className="mx-auto w-16 h-16 bg-green-100 rounded-full flex items-center justify-center mb-4">
            <CheckCircle2 className="w-10 h-10 text-green-600" />
          </div>
          <h2 className="text-2xl font-black text-gray-900 mb-6">Payment Successful!</h2>
      ...
      {/* Sale Details Modal (POS Terminal) */}
      <Dialog open={isViewSaleDetailOpen} onOpenChange={setIsViewSaleDetailOpen}>
        <DialogContent className="sm:max-w-xl max-h-[90vh] overflow-y-auto rounded-2xl border-0 shadow-2xl p-6 bg-white">
          <DialogHeader>
            <DialogTitle className="text-xl font-black text-gray-900">
              Sale Details - {viewSaleDetail?.invoiceNo || viewSaleDetail?.receiptNumber}
            </DialogTitle>
            <DialogDescription className="text-xs text-gray-500 font-medium">
              {viewSaleDetail && new Date(viewSaleDetail.createdAt).toLocaleString()} | Cashier: {viewSaleDetail?.cashier || user?.name || 'Super Admin'}
            </DialogDescription>
          </DialogHeader>
          ...
          {viewSaleDetail && (
            <div className="space-y-6 mt-4">
              <div className="grid grid-cols-2 gap-4 bg-gray-50 p-4 rounded-xl text-sm">
                ...
                <div>
                  <span className="text-gray-400 block text-xs font-medium">Order Type</span>
                  <span className="font-bold text-gray-900">{viewSaleDetail.orderType || 'Takeaway'}</span>
                </div>
                <div>
                  <span className="text-gray-400 block text-xs font-medium">Payment Method</span>
                  <span className="font-bold text-gray-900">{viewSaleDetail.paymentMethod || 'Cash'}</span>
                </div>
                <div>
                  <span className="text-gray-400 block text-xs font-medium">Status</span>
                  <span className="inline-block mt-0.5 px-2.5 py-0.5 rounded-full text-xs font-bold bg-green-100 text-green-700">
                    {viewSaleDetail.status || 'Completed'}
                  </span>
                </div>
              </div>

              <div>
                <h3 className="font-bold text-gray-900 mb-3 text-sm">Items Purchased</h3>
                <div className="border border-gray-100 rounded-xl overflow-hidden">
                  <table className="w-full text-sm text-left">
                    ...
                    <tbody className="divide-y divide-gray-100">
                      {viewSaleDetail.items?.map((item: any, i: number) => (
                        <tr key={i} className="hover:bg-gray-50/50">
                          <td className="py-3 px-3">
                            <div className="font-bold text-gray-900">{item.name}</div>
                            {(item.variant || (item.addons && item.addons.length > 0) || item.note) && (
                          ...
                          <td className="py-3 px-3 text-center font-medium text-gray-700">{item.quantity}</td>
                          <td className="py-3 px-3 text-right font-bold text-gray-900">
                            Rs. {(item.totalPrice || (item.basePrice * item.quantity))?.toFixed(2)}
                          </td>
                        </tr>
                      ))}
                    </tbody>
              ...
              <div className="space-y-1 text-sm text-right pt-4 border-t border-gray-100">
                <div className="text-gray-500 font-medium">
                  Subtotal: <span className="font-bold text-gray-900">Rs. {(viewSaleDetail.subtotal || viewSaleDetail.total)?.toFixed(2)}</span>
                </div>
                {viewSaleDetail.discount > 0 && (
                  <div className="text-red-500 font-medium">
                    Discount: <span className="font-bold">-Rs. {viewSaleDetail.discount?.toFixed(2)}</span>
                  </div>
                )}
                <div className="text-xl font-black text-gray-900 pt-1">
                  Total: Rs. {viewSaleDetail.total?.toFixed(2)}
                </div>
              </div>
            </div>
          )}
          ...
          <DialogFooter className="mt-4">
            <Button variant="outline" className="font-bold rounded-xl px-6 bg-gray-900 text-white hover:bg-gray-800" onClick={() => setIsViewSaleDetailOpen(false)}>
              Close
            </Button>
          </DialogFooter>
        </DialogContent>
